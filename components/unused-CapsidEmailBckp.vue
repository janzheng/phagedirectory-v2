...
<template>
  ...
  <div class="Periodical Email-section">
    ...
    <div  v-for="issue of issues" :key="issue.id" v-if="(showPreview && issue.fields.isPreview) || issue.fields.isPublished">
      ...
      <div class="test">This is a temporary test!!!!</div>

      <div>
        <span style="font-size:16px"><strong>{{ issue.fields['Name'] }}</strong>, {{ issue.fields['Date'] | niceDate }}</span><br />
        <span style="font-size:16px"><a :href="`${issue.fields['URL']}`" target="_blank">Read on Phage Directory</a></span>
      </div>
      ...
      <div class="Periodical-updates" v-if="getUpdates(issue)" style="box-shadow: 0px 4px 8px rgba(70, 70, 70, .1); color: #333333; background-color: #FCFCFC; border-radius: 4px; padding: 30px; margin-bottom: 15px;">
        ...
        <div class="Periodical-update-item _margin-bottom" v-for="update of getUpdates(issue)" :key="update.fields['Name']" v-if="update && update.fields['isPublished']">
          ...
          <div class="_margin-top-half" v-if="update.fields['Tags']">
            <span v-for="tag of update.fields.Tags" :key="tag"><span style="background-color:#eeeeee; border-radius:4px; border:1px solid #eeeeee; color:#000000; display:inline-block; font-size:14px; outline:none!important; padding:1px 8px; text-decoration:none" :class="tag == 'Sponsor' ? '--sponsor' : ''" >{{ tag }}</span>&nbsp;</span>
          </div><br/>
        </div>
        <br/>
