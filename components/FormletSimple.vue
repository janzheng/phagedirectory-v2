  ...
  <div class="Formlet item--fields" v-if="inputs">
    <div class="item--field" v-for="input in inputs" :class="input.required ? '--required' : '' " :key="input.name">
      ...
      <div class="_form-control" v-if="input.type==INPUT_TYPES.TEXT">
        ...
        <input v-model.trim="fieldData[input.name]" @input="validate(fieldData[input.name])" class="_form-input --width-full" :class="inputClasses" type="text" :name="input.name" :id="input.name" :placeholder="input.placeholder" :required="input.required"/>
      ...
      <div class="_form-control" v-if="input.type==INPUT_TYPES.EMAIL">
        <label :for="input.name" class="_form-label" v-html="markdown(input.label)"></label>
        <label :for="input.name" class="_form-label _form-desc" v-html="markdown(input.description)" v-if="markdown(input.description)"></label>
        <input v-model.trim="fieldData[input.name]" @input="validate(fieldData[input.name])" class="_form-input --width-full" :class="inputClasses" type="email" :name="input.name" :id="input.name" :placeholder="input.placeholder" :required="input.required"/>
        <!-- <label class="_error" v-if="isFieldInvalid(input)">{{ errorMessage(input) }}</label> -->
      </div>
      ...
      <div class="_form-control" v-if="input.type==INPUT_TYPES.NUMBER">
        <label :for="input.name" class="_form-label" v-html="markdown(input.label)"></label>
        <label :for="input.name" class="_form-label _form-desc" v-html="markdown(input.description)" v-if="markdown(input.description)"></label>
        <input v-model.trim="fieldData[input.name]" @input="validate(fieldData[input.name])" class="_form-input --width-full" :class="inputClasses" type="number" :name="input.name" :id="input.name" :placeholder="input.placeholder" :required="input.required"/>
        <!-- <label class="_error" v-if="isFieldInvalid(input)">{{ errorMessage(input) }}</label> -->
      </div>
      ...
      <div class="_form-control" v-if="input.type==INPUT_TYPES.TEXTAREA">
        <label :for="input.name" class="_form-label" v-html="markdown(input.label)"></label>
        <label :for="input.name" class="_form-label _form-desc" v-html="markdown(input.description)" v-if="markdown(input.description)"></label>
        <textarea v-model.trim="fieldData[input.name]" @input="validate(fieldData[input.name])" rows="4" :id="input.name" class="_form-input" type="text" :name="input.name" :placeholder="input.placeholder" :required="input.required"></textarea>
        <!-- <label class="_error" v-if="isFieldInvalid(input)">{{ errorMessage(input) }}</label> -->
      </div>
...
export default {
  ...
  methods: {
    ...
    //   this.$v.$touch()
    // },
    validate (value) { // delay form validation 
      const _this = this
      this.isValidating = true

      console.log('formlet validate:', this.fieldData)
      _this.$emit('handler', this.fieldData)
